    ...
    return (
        <header className="h-[70px] bg-white border-b border-[#f0f0f0] flex items-center justify-between px-4 md:px-8 sticky top-0 z-40">
            <div className="flex items-center gap-4 md:gap-6">
                ...
                <h1 className="text-[14px] md:text-[15px] font-black text-[#1E1E1E] tracking-tight truncate max-w-[150px] md:max-w-none">{title}</h1>
                <a
                    href="https://consulting.wagetrail.com/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hidden lg:flex flex-col items-center gap-0.5 bg-[#2C76FF] text-white px-4 py-1.5 rounded-xl text-[10px] font-black border border-[#2C76FF]/30 hover:bg-[#1a60e6] transition-all ml-4 group shadow-lg shadow-[#2C76FF]/10"
                >
            ...
            <div className="flex items-center gap-4 relative" ref={dropdownRef}>
                ...
                >
                    <div className="w-9 h-9 bg-[#2C76FF] rounded-lg flex items-center justify-center text-white shadow-lg overflow-hidden">
                        <User size={20} />
                    </div>
                    <div className="hidden md:block text-left">
                        <p className="text-[12px] font-black text-[#1E1E1E] leading-none mb-0.5">{user?.email?.split('@')[0] || 'User'}</p>
                ...
                {showDropdown && (
                    <div className="absolute top-[calc(100%+8px)] right-0 w-48 bg-white border border-[#f0f0f0] rounded-2xl shadow-xl py-2 animate-in fade-in slide-in-from-top-2 duration-200 z-50">
                        <button
                            onClick={() => navigate('/')}
                            className="w-full flex items-center gap-3 px-4 py-2.5 text-[13px] font-bold text-gray-700 hover:bg-[#fafafa] hover:text-[#2C76FF] transition-all"
                        >
                            <Home size={16} />
                            Home
                        </button>
